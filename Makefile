...
REPO_PATH   ?= sigs.k8s.io/cluster-api-provider-kubevirt
...
ifeq ($(NO_DOCKER), 1)
  ...
  DOCKER_CMD := docker run --rm -e CGO_ENABLED=1 -v "$(PWD)":/go/src/sigs.k8s.io/cluster-api-provider-kubevirt:Z -w /go/src/sigs.k8s.io/cluster-api-provider-aws openshift/origin-release:golang-1.13
...
lint: ## Go lint your code
	hack/go-lint.sh -min_confidence 0.3 $$(go list -f '{{ .ImportPath }}' ./... | grep -v -e 'sigs.k8s.io/cluster-api-provider-kubevirt/test' -e 'sigs.k8s.io/cluster-api-provider-kubevirt/pkg/cloud/kubevirt/client/mock')
